...
namespace _20260327_01_OffsetGeoShape
{
    ...
    public class GeoShapeLight : Shape
    {
        private Geometry? _cachedGeometry;
        ...
        public static readonly DependencyProperty IsOffsetProperty =
            DependencyProperty.Register(nameof(IsOffset), typeof(bool), typeof(GeoShapeLight), new FrameworkPropertyMetadata(false, OnPointsChanged));
        ...
        public static readonly DependencyProperty StrokePenProperty =
            DependencyProperty.Register(nameof(StrokePen), typeof(Pen), typeof(GeoShapeLight), new FrameworkPropertyMetadata(null, OnStrokePenChanged));

        private static void OnStrokePenChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is GeoShapeLight GeoShape)
            {
                GeoShape._cachedGeometry = null;
            }
        }
        ...
        public static readonly DependencyProperty PointsProperty =
            DependencyProperty.Register(nameof(Points), typeof(PointCollection), typeof(GeoShapeLight),
                new FrameworkPropertyMetadata(null, OnPointsChanged));

        // OnPointsChangedでの処理はこれで十分？
        private static void OnPointsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is GeoShapeLight shape)
            {
        ...
        protected override Geometry DefiningGeometry
        {
            get
            {
                ...
                if (IsOffset)
                {
                    var bounds = geo.GetRenderBounds(StrokePen);
                    // 描画位置が(0,0)になるようにオフセットしたGeometryを作成して返す
                    var transformedGeo = geo.Clone();
                    transformedGeo.Transform = new TranslateTransform(-bounds.X, -bounds.Y);
                    transformedGeo.Freeze(); // 要る？
                    _cachedGeometry = transformedGeo;
                }
                else
